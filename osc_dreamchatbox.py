...
Features:
- Personal Status: sends your text to the VRChat chatbox on a timer
- MediaPlay: shows the song you're currently listening to (Spotify,
  YT Music, browser, any player) via MPRIS / D-Bus
- SendToVRChat master switch
- Debug console (live log of everything that gets sent)
- "Slim Chatbox" mode (default ON): appends the invisible control
  characters \u0003\u001f so VRChat renders a slim bar instead of the
  huge box. (Known from MagicChatbox as the hidden "BlankEgg" egg.)
...
from ui_main import (STYLE, ToggleSwitch, ToggleLabel, DebugConsole, DragHandle,
                     EmojiPopup)
from hardware import HardwareMonitor
from speechtotext import SpeechWorker, LANGUAGES, OUTPUT_LANGUAGES
from oscrouter import OSCRouter
...
# ----------------------------------------------------------------------------
# MPRIS media fetcher (Spotify, YT Music, browsers, VLC, ... via D-Bus)
# ----------------------------------------------------------------------------
class MediaFetcher:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.router = OSCRouter(self.log)
        self.route_procs = {}      # program name -> subprocess.Popen
        self.prog_buffers = {}     # name -> deque of recent output lines
        self.prog_queues = {}      # name -> Queue with new lines
        self.prog_consoles = {}    # name -> DebugConsole window
        ...
        self.route_timer.timeout.connect(self.refresh_route_list)
        self.prog_out_timer = QTimer(self)
        self.prog_out_timer.timeout.connect(self.drain_prog_output)
    ...
    def load_config(self):
        defaults = {
            ...
            "route_port": 9101,
            "route_blocked": [],
            "route_programs": [],  # [{"name","type":"file"|"cmd","cmd"}]
            "stt_block_saved": [],
            "aio_active": False,
            "aio_count": 1,
            "aio_rotate": False,
        ...
        for p in defaults.get("route_programs", []):
            p.setdefault("debug", False)
    ...
    def build_ui(self):
        ...
        self.pages.addWidget(self._wrap_scroll(self.build_routing_page()))
    ...
    def build_routing_page(self):
        page = QWidget()
        layout = QVBoxLayout(page)
        layout.setContentsMargins(24, 20, 24, 20)
        layout.setSpacing(16)

        title = QLabel("OSC Routing")
        title.setObjectName("pagetitle")
        layout.addWidget(title)
        ...
        head.addWidget(ht)
        head.addStretch()
        self.toggle_route = ToggleSwitch()
        self.toggle_route.toggled.connect(self.on_route_toggled)
        head.addWidget(self.toggle_route)
        head.addWidget(ToggleLabel("Active", self.toggle_route))
        c.addLayout(head)

        rdesc = QLabel("Other OSC programs (OSC Leash, face tracking, \u2026) "
                       "send to THIS port instead of VRChat directly. "
                       "OSC-DreamChatbox catches their packets and forwards "
                       "everything bundled through one connection to VRChat \u2013 "
                       "avoiding port conflicts and bugs.")
        rdesc.setObjectName("dim")
        rdesc.setWordWrap(True)
        c.addWidget(rdesc)

        port_row = QHBoxLayout()
        port_row.addWidget(QLabel("Listen port:"))
        self.route_port_spin = QSpinBox()
        self.route_port_spin.setRange(1024, 65535)
        self.route_port_spin.setFixedWidth(100)
        self.route_port_spin.valueChanged.connect(self.on_route_port)
        port_row.addWidget(self.route_port_spin)
        port_row.addStretch()
        c.addLayout(port_row)

        self.route_hint = QLabel("")
        self.route_hint.setObjectName("dim")
        self.route_hint.setWordWrap(True)
        c.addWidget(self.route_hint)
        layout.addWidget(card)

        # ----- Programm-Liste -----
        lcard = QFrame()
        lcard.setObjectName("card")
        lc = QVBoxLayout(lcard)
        lc.setContentsMargins(16, 14, 16, 16)
        lc.setSpacing(8)
        lt = QLabel("Connected programs")
        lt.setObjectName("cardtitle")
        lc.addWidget(lt)
        ld = QLabel("Programs that sent OSC data to the router. Uncheck one "
                    "to stop forwarding its packets to VRChat.")
        ld.setObjectName("dim")
        ld.setWordWrap(True)
        lc.addWidget(ld)

        self.route_list_box = QFrame()
        self.route_list_box.setObjectName("innerbox")
        self.route_list_layout = QVBoxLayout(self.route_list_box)
        self.route_list_layout.setContentsMargins(14, 10, 14, 12)
        self.route_list_layout.setSpacing(6)
        self.route_empty_lbl = QLabel("No programs detected yet.")
        self.route_empty_lbl.setObjectName("dim")
        self.route_list_layout.addWidget(self.route_empty_lbl)
        lc.addWidget(self.route_list_box)
        layout.addWidget(lcard)

        # ----- manually managed programs -----
        pcard = QFrame()
        pcard.setObjectName("card")
        pc = QVBoxLayout(pcard)
        pc.setContentsMargins(16, 14, 16, 16)
        pc.setSpacing(8)
        pt = QLabel("Managed programs")
        pt.setObjectName("cardtitle")
        pc.addWidget(pt)
        pdsc = QLabel("Add OSC programs manually and start them from here. "
                      "They get the router port handed over via the "
                      "environment (OSC_PORT / VRC_OSC_PORT). For AppImages "
                      "that can't change their port at all: set the router's "
                      "listen port to 9000 and move VRChat to another port "
                      "with the launch option "
                      "--osc=9002:127.0.0.1:9003 (then set the OSC target "
                      "port under Options to 9002) \u2013 every program lands "
                      "in the router automatically.")
        pdsc.setObjectName("dim")
        pdsc.setWordWrap(True)
        pc.addWidget(pdsc)

        add_row = QHBoxLayout()
        self.prog_method_combo = QComboBox()
        self.prog_method_combo.addItem("Select file (AppImage / .sh)", "file")
        self.prog_method_combo.addItem("CMD command", "cmd")
        self.prog_method_combo.currentIndexChanged.connect(
            self.on_prog_method_changed)
        add_row.addWidget(self.prog_method_combo)
        self.prog_cmd_input = QLineEdit()
        self.prog_cmd_input.setPlaceholderText("e.g.  python3 ~/tools/leash.py --debug")
        self.prog_cmd_input.setVisible(False)
        add_row.addWidget(self.prog_cmd_input, 1)
        self.prog_browse_btn = QPushButton("Browse \u2026")
        self.prog_browse_btn.setObjectName("linkbtn")
        self.prog_browse_btn.setFixedHeight(30)
        self.prog_browse_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.prog_browse_btn.clicked.connect(self.on_prog_browse)
        add_row.addWidget(self.prog_browse_btn)
        self.prog_add_btn = QPushButton("Add")
        self.prog_add_btn.setObjectName("sendbtn")
        self.prog_add_btn.setFixedSize(64, 30)
        self.prog_add_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.prog_add_btn.clicked.connect(self.on_prog_add)
        add_row.addWidget(self.prog_add_btn)
        add_row.addStretch()
        pc.addLayout(add_row)

        self.prog_list_box = QFrame()
        self.prog_list_box.setObjectName("innerbox")
        self.prog_list_layout = QVBoxLayout(self.prog_list_box)
        self.prog_list_layout.setContentsMargins(14, 10, 14, 12)
        self.prog_list_layout.setSpacing(6)
        pc.addWidget(self.prog_list_box)
        layout.addWidget(pcard)

        layout.addStretch()
        self.rebuild_prog_list()
        return page

    # -------------------------------------------------- managed programs
    def on_prog_method_changed(self, idx):
        is_cmd = self.prog_method_combo.currentData() == "cmd"
        self.prog_cmd_input.setVisible(is_cmd)
        self.prog_browse_btn.setVisible(not is_cmd)

    def on_prog_browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select program", str(Path.home()),
            "Programs (*.AppImage *.sh *.py);;All files (*)")
        if path:
            self._add_program(Path(path).name, "file", path)

    def on_prog_add(self):
        if self.prog_method_combo.currentData() == "cmd":
            cmd = self.prog_cmd_input.text().strip()
            if cmd:
                name = cmd.split()[0].split("/")[-1]
                self._add_program(name, "cmd", cmd)
                self.prog_cmd_input.clear()
        else:
            self.on_prog_browse()

    def _add_program(self, name, ptype, cmd):
        # unique name
        base, i = name, 2
        names = {p["name"] for p in self.cfg["route_programs"]}
        while name in names:
            name = f"{base} ({i})"; i += 1
        self.cfg["route_programs"].append(
            {"name": name, "type": ptype, "cmd": cmd})
        self.save_config()
        self.log(f"OSC Routing: program added \u2013 {name}")
        self.rebuild_prog_list()

    def on_prog_remove(self, name):
        self.on_prog_stop(name)
        self.cfg["route_programs"] = [
            p for p in self.cfg["route_programs"] if p["name"] != name]
        self.save_config()
        self.rebuild_prog_list()

    def on_prog_start(self, name):
        prog = next((p for p in self.cfg["route_programs"]
                     if p["name"] == name), None)
        if prog is None or self.prog_running(name):
            return
        env = dict(os.environ)
        env["OSC_PORT"] = str(self.cfg["route_port"])
        env["VRC_OSC_PORT"] = str(self.cfg["route_port"])
        env["VRCHAT_OSC_PORT"] = str(self.cfg["route_port"])
        try:
            popen_kw = dict(env=env, stdout=subprocess.PIPE,
                            stderr=subprocess.STDOUT, text=True,
                            bufsize=1, errors="replace")
            if prog["type"] == "cmd":
                proc = subprocess.Popen(["/bin/sh", "-c", prog["cmd"]],
                                        **popen_kw)
            else:
                path = prog["cmd"]
                if not os.access(path, os.X_OK):
                    try:
                        ...
                        pass
                proc = subprocess.Popen([path], **popen_kw)
            self.route_procs[name] = proc
            # capture the program's output in the background (a ring
            # buffer keeps the last 400 lines; the pipe is always drained
            # so the child never blocks on a full buffer)
            buf = self.prog_buffers.setdefault(name, deque(maxlen=400))
            q = self.prog_queues.setdefault(name, _queue.Queue())

            def reader(p=proc, b=buf, qq=q):
                try:
            ...
            self.prog_out_timer.start(400)
            self.log(f"OSC Routing: started \"{name}\" (PID {proc.pid})")
            if prog.get("debug"):
                self.open_prog_console(name)
        except Exception as e:
            self.log(f"OSC Routing: could not start \"{name}\": {e}")
        self.rebuild_prog_list()

    def on_prog_stop(self, name):
        proc = self.route_procs.pop(name, None)
        if proc is not None and proc.poll() is None:
            try:
                ...
                pass
            self.log(f"OSC Routing: stopped \"{name}\"")
        self.rebuild_prog_list()

    def open_prog_console(self, name):
        con = self.prog_consoles.get(name)
        if con is None:
            con = DebugConsole(f"{name}")
            self.prog_consoles[name] = con
            # show the buffered history first
            for line in self.prog_buffers.get(name, ()):
                con.log(line)
            # drop what's still in the queue (already shown via buffer)
            q = self.prog_queues.get(name)
            if q:
                while not q.empty():
                    try:
                        q.get_nowait()
    ...
    def on_prog_debug(self, name, on):
        for p in self.cfg["route_programs"]:
            if p["name"] == name:
                p["debug"] = on
        self.save_config()
        if on:
    ...
    def drain_prog_output(self):
        any_running = False
        for name, q in self.prog_queues.items():
            con = self.prog_consoles.get(name)
            while not q.empty():
                try:
                    line = q.get_nowait()
                    ...
                    break
                if con is not None and con.isVisible():
                    con.log(line)
            if self.prog_running(name):
                any_running = True
        if not any_running:
            self.prog_out_timer.stop()

    def prog_running(self, name):
        proc = self.route_procs.get(name)
        return proc is not None and proc.poll() is None

    def rebuild_prog_list(self):
        while self.prog_list_layout.count():
            item = self.prog_list_layout.takeAt(0)
            wdg = item.widget()
            if wdg is not None:
                wdg.deleteLater()
        progs = self.cfg["route_programs"]
        if not progs:
            lbl = QLabel("No programs added yet.")
            lbl.setObjectName("dim")
            self.prog_list_layout.addWidget(lbl)
            return
        for p in progs:
            row_w = QWidget()
            row = QHBoxLayout(row_w)
            row.setContentsMargins(0, 0, 0, 0)
            row.setSpacing(8)
            running = self.prog_running(p["name"])
            state = QLabel("\u25CF" if running else "\u25CB")
            state.setStyleSheet(
                f"color: {'#7fc97f' if running else '#7a8290'};")
            row.addWidget(state)
            nlbl = QLabel(p["name"])
            row.addWidget(nlbl)
            clbl = QLabel(p["cmd"] if len(p["cmd"]) < 42
                          else p["cmd"][:41] + "\u2026")
            clbl.setObjectName("dim")
            row.addWidget(clbl)
            row.addStretch()
            dbg = QCheckBox("Debug")
            dbg.setChecked(p.get("debug", False))
            dbg.setToolTip("Show a console window with this program's output")
            dbg.toggled.connect(
                lambda on, n=p["name"]: self.on_prog_debug(n, on))
            row.addWidget(dbg)
            act = QPushButton("Stop" if running else "Start")
            act.setObjectName("sendbtn" if not running else "linkbtn")
            act.setFixedSize(60, 26)
            act.setCursor(Qt.CursorShape.PointingHandCursor)
            if running:
                act.clicked.connect(lambda _, n=p["name"]: self.on_prog_stop(n))
            else:
                act.clicked.connect(lambda _, n=p["name"]: self.on_prog_start(n))
            row.addWidget(act)
            rm = QPushButton("\u2715")
            ...
            rm.clicked.connect(lambda _, n=p["name"]: self.on_prog_remove(n))
            row.addWidget(rm)
            self.prog_list_layout.addWidget(row_w)

    # ------------------------------------------------------- routing logic
    def on_route_toggled(self, on):
        self.cfg["route_enabled"] = on
        self.save_config()
        if on:
            self.router.set_target(self.cfg["osc_ip"], self.cfg["osc_port"])
            self.router.start(self.cfg["route_port"],
                              self.cfg.get("route_blocked", []))
            self.route_timer.start(1000)
            self.route_hint.setText(
                f"Configure other OSC programs to send to "
                f"127.0.0.1:{self.cfg['route_port']} \u2013 the router "
                f"forwards to {self.cfg['osc_ip']}:{self.cfg['osc_port']}.")
            self.log(f"OSC Routing: ACTIVE on port {self.cfg['route_port']}")
        else:
            self.router.stop()
            self.route_timer.stop()
            self.route_hint.setText("")
            self.log("OSC Routing: inactive")
        self.refresh_route_list()

    def on_route_port(self, val):
        self.cfg["route_port"] = val
        self.save_config()
        if self.cfg["route_enabled"]:
            # restart on the new port
            self.on_route_toggled(True)

    def on_route_source_toggled(self, key, forward_on):
        blocked = not forward_on
        self.router.set_blocked(key, blocked)
        bl = set(self.cfg.get("route_blocked", []))
        (bl.add if blocked else bl.discard)(key)
        self.cfg["route_blocked"] = sorted(bl)
        self.save_config()
        self.log(f"OSC Routing: {key} {'BLOCKED' if blocked else 'forwarding'}")

    def refresh_route_list(self):
        snap = self.router.snapshot()
        if self.router.error:
            self.route_hint.setText(self.router.error)
        # rebuild only when the set of sources changes; otherwise just
        # update the info labels (keeps checkboxes clickable)
        keys = sorted(snap.keys())
        if getattr(self, "_route_keys", None) != keys:
            ...
            for key in keys:
                ...
                row.setSpacing(8)
                chk = QCheckBox(key)
                chk.setChecked(not snap[key]["blocked"])
                chk.toggled.connect(
                    lambda on, k=key: self.on_route_source_toggled(k, on))
                row.addWidget(chk)
                info = QLabel("")
                info.setObjectName("dim")
                row.addWidget(info)
                row.addStretch()
                self.route_list_layout.addWidget(row_w)
                self._route_rows[key] = info
        # keep the Start/Stop states of managed programs fresh
        for name, proc in list(self.route_procs.items()):
            if proc.poll() is not None:
                self.route_procs.pop(name, None)
                self.log(f"OSC Routing: \"{name}\" exited")
                self.rebuild_prog_list()
        for key, info in getattr(self, "_route_rows", {}).items():
            s = snap.get(key)
            if s:
                age = time.time() - s["last_seen"]
                live = "\u25CF live" if age < 3 else f"{age:.0f}s ago"
                info.setText(f"{s['last_addr']} \u00b7 {s['count']} msgs "
                             f"\u00b7 {live}")
    ...
    def apply_config_to_ui(self):
        ...
        self.toggle_route.setChecked(self.cfg["route_enabled"])
        self.route_port_spin.setValue(self.cfg["route_port"])
    ...
    def current_status_text(self):
        """Returns the currently shown status text (rotates through the
        non-empty texts every status_cycle_sec seconds)."""
    ...
    def _media_values(self, info):
        ...
        if info["length"] > 0:
            ...
            filled = round(frac * SONGBAR_LEN)
            bar = "\u2593" * filled + "\u2591" * (SONGBAR_LEN - filled)
        time_str = (f"{fmt_time(info['position'])}/{fmt_time(info['length'])}"
                    if info["length"] > 0 else fmt_time(info["position"]))
        return {
            "artist": info["artist"] if c["media_show_artist"] else None,
            "title": t if c["media_show_title"] else None,
            "position": fmt_time(info["position"]),
            "length": fmt_time(info["length"]) if info["length"] > 0 else None,
    ...
    def build_media_lines(self):
        ...
        if self.cfg["media_show_bar"] and info["length"] > 0:
            frac = min(1.0, max(0.0, info["position"] / info["length"]))
            filled = round(frac * SONGBAR_LEN)
            lines.append("▓" * filled + "░" * (SONGBAR_LEN - filled))
        if lines and self.cfg["media_icon"]:
            lines[0] = f"\U0001F3B5 {lines[0]} \U0001F3B5"
        return lines
    ...
    def closeEvent(self, ev):
        self.stt.stop()
        self.router.stop()
        for name in list(self.route_procs):
            self.on_prog_stop(name)
        for con in self.prog_consoles.values():
            con.close()
        self._save_timer.stop()
        self._write_config()
