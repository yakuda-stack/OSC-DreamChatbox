...
class ConfigMixin:
    def load_config(self):
        ...
        defaults = {
            ...
            # A fresh install arrives set up rather than blank: a clock
            # on top, the app name underneath, Double frame. An empty card
            # with thirteen switches teaches nobody what it does, and
            # every one of these is a click away from something else.
            # "box_active" is the exception - see the note further down.
            "box_active": True,
            ...
            # Kept short on purpose - a frame line long enough to wrap in
            # VRChat turns one line into two and eats the chatbox
            "box_width_top": 7,
            "box_width_bottom": 3,
        ...
        # The frame is on for a FRESH install only. A config that predates
        # the Custom Box has no box keys, so it would otherwise inherit
        # the new default and grow a frame on update - changing what
        # somebody's chatbox looks like without being asked. Their card
        # still comes up filled in with everything above, so switching it
        # on is one click and looks exactly like a new install.
        if stored and "box_active" not in stored:
            defaults["box_active"] = False
        else:
            defaults["box_active"] = bool(defaults.get("box_active", True))
