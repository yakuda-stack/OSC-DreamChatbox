...
from core.constants import (
    APP_NAME, CHATBOX_INPUT, CHATBOX_LIMIT, CHATBOX_WRAP_CELLS, OSC_MIN_SEND_GAP_SEC, OSC_RATE_MAX_SENDS, OSC_RATE_WINDOW_SEC, SLIM_SUFFIX, VERSION)
...
from core.textutils import (
    CUSTOM_STYLE_INDEX, DEFAULT_CUSTOM_BAR, TIME_POS_LINE, fmt_time, fmt_time_hm, wrap_cells)
...
class MainWindow(ConfigMixin, AppsPageMixin, CustomBoxMixin,
    ...
    def update_preview(self):
        ...
        if paused:
            ...
            text = self.build_payload()
        # shown wrapped the way the chatbox wraps, so a line that will
        # break in VRChat also breaks here. Showing the raw payload made
        # the app look right while the headset showed two lines.
        wrapped = wrap_cells(text, CHATBOX_WRAP_CELLS) if text else []
        self.preview_label.setText("\n".join(wrapped) if wrapped
                                   else "[Status Text goes here]")
        ...
        # how many payload lines the chatbox had to break. Worth its own
        # note: the character counter stays green while a line wraps,
        # because wrapping costs no characters - it costs a line.
        broke = len(wrapped) - len(text.split("\n")) if text else 0
        if not text:
            ...
            self.char_count_lbl.setText(
                f"{n}/{CHATBOX_LIMIT}  ·  ⤶ {broke} "
                f"line{'s' if broke > 1 else ''} wrap")
            self.char_count_lbl.setStyleSheet("color: #d9884a; font-size: 12px;")
        else:
