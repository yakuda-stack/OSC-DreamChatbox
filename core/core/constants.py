...
#: Roughly how many character cells fit on one line of the VRChat
#: chatbox before it wraps. Unlike CHATBOX_LIMIT this is an ESTIMATE, not
#: a documented number: the chatbox font is not monospaced, so the real
#: wrap point depends on which characters are on the line. It is used to
#: soft-wrap the preview so a line that will break in VRChat also breaks
#: here, instead of the app showing one line and VRChat showing two.
#: Tune it if the preview and your headset disagree.
CHATBOX_WRAP_CELLS = 24
