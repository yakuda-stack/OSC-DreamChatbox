...
``settings`` entries (type text | bool | int | slider | choice | group)
...
Two of those types are about structure rather than about a value:
...
LEAF_SETTING_TYPES = ("text", "bool", "int", "slider", "choice")
# purely structural: a collapsible block around other settings
GROUP_TYPE = "group"
SETTING_TYPES = LEAF_SETTING_TYPES + (GROUP_TYPE,)
    ...
    def _parse_schema(raw, _depth=0, _seen=None):
        ...
        for item in raw:
            ...
            if not key or key in seen or kind not in SETTING_TYPES:
                continue
            if kind == GROUP_TYPE and _depth >= MAX_GROUP_DEPTH:
                continue
            seen.add(key)
            entry = {
                "key": key,
