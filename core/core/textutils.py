...
from core.boxstyle import cells
...
def wrap_cells(text: str, width: int):
    """Soft-wraps `text` the way the VRChat chatbox does: by width, not
    by character count, so a line of emoji breaks earlier than a line of
    letters.

    Returns a list of lines. Breaks between words where it can and inside
    a word only when a single word is wider than the whole line - the
    chatbox does the same, and a frame line is exactly that case, one
    long unbreakable run of box characters.

    An approximation on purpose: the chatbox font is not monospaced, so
    nothing short of rendering it can be exact. It is here to show that a
    line WILL break, not to predict the pixel it breaks at.
    """
    ...
    for line in (text or "").split("\n"):
        ...
        for word in line.split(" "):
            ...
            if current:
                out.append(current)
                current = ""
            # a single word wider than the line has to be cut somewhere
            while cells(word) > width:
                cut = ""
                for char in word:
                    if cells(cut + char) > width:
                        break
                    cut += char
                out.append(cut)
                word = word[len(cut):]
