...
- **A fresh install arrives set up, not blank:**
  ...
  alignment off because those two widths are deliberately different. The
  widths are short on purpose: a frame line long enough to wrap turns one line
  into two and eats the chatbox. A card
  with thirteen switches and an empty preview teaches nobody what it does, and
  every one of these settings is one click away from something else.

  **The frame does not switch itself on for an existing install.** A config
  written before v1.3.2 has no box keys and would otherwise inherit the new
  default, growing a frame on somebody's chatbox without being asked. Those
  configs come up with the card switched off but fully filled in, so turning
  it on is one click and looks exactly like a new install.
...
- **The preview now wraps the way the chatbox wraps.** It showed the raw
  payload, so a line too wide for the VRChat chatbox looked fine in the app
  and arrived as two lines in the headset - which is exactly the thing the
  preview exists to prevent. It is soft-wrapped by character *cells* rather
  than character count, so a line of emoji breaks earlier than a line of
  letters, and the counter says how many lines broke:
  `64/144 · ↶ 1 line wrap`. Worth its own note because the character counter
  stays green while it happens - wrapping costs no characters, it costs a
  line.

  `CHATBOX_WRAP_CELLS` is an estimate, and marked as one: the chatbox font is
  not monospaced, so nothing short of rendering it can be exact. It is there
  to show that a line *will* break, not to predict where. One constant to tune
  if the preview and the headset disagree.
...
- New files: `core/textstyle.py` (the maps, the `_"keep me"_` handling and the
  ...
  `core/plugins.py`, `core/textutils.py`, `ui/pages/plugins_page.py`,
  `ui/pages/apps_page.py`, `ui/mainwindow.py`, `ui/config_mixin.py`.
