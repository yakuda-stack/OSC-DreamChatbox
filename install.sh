...
# The app needs PortAudio (microphone / Speech to Text) and, on X11/non-KDE
# desktops, the Qt xcb-cursor library (otherwise Qt6 fails with
# "Could not load the Qt platform plugin xcb"). Non-Arch users (e.g. Linux
# Mint) usually have to add these by hand, so we detect the distro and offer
# to install them.
install_system_deps() {
    ...
    if [ -t 0 ] && command -v sudo >/dev/null 2>&1; then
        ...
        case "$answer" in
            ...
            *)
                # never let a package failure abort the whole installer
                $installcmd $pkgs || \
...
if [ -f "/usr/share/applications/osc-dreamchatbox.desktop" ] \
   || [ -f "/usr/local/share/applications/osc-dreamchatbox.desktop" ]; then
    ...
    # Install the icon into the hicolor theme and reference it by NAME.
    # KDE/Wayland taskbars resolve the window icon via the theme name from
    # the .desktop; absolute Icon= paths often don't render there.
