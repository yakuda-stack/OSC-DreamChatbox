...
class AppsPageMixin:
    def build_apps_page(self):
        ...
        # ---- AFK ----------------------------------------------------
        # The two switches themselves live in the right-hand column, on
        # purpose: they are things you flip while playing. Everything
        # about HOW the AFK line looks is a setting, so it sits here -
        # folded away, so the sidebar stays two rows tall instead of
        # growing a text field, a dropdown and two checkboxes.
        self.afk_expander = QPushButton("\u25B8  Customize AFK text")
        self.afk_expander.setObjectName("expander")
        self.afk_expander.setCursor(Qt.CursorShape.PointingHandCursor)
        self.afk_expander.clicked.connect(self.on_afk_expand)
        sc.addWidget(self.afk_expander)

        self.afk_container = QWidget()
        ac = QVBoxLayout(self.afk_container)
        ac.setContentsMargins(0, 0, 0, 0)
        ac.setSpacing(8)
        self.afk_container.setVisible(False)

        afk_info = QLabel(
            "\u2139 The two AFK switches sit under the Preview, on the "
            "right. <b>Detect AFK</b> watches VRChat's own AFK avatar "
            "parameter \u2013 headset off, or the window left alone \u2013 "
            "and swaps the chatbox over by itself. <b>I'm AFK</b> does the "
            "same for as long as you hold it on, with no detection "
            "involved.<br><br>"
            "Both work whether or not Personal Status is switched on; "
            "this block is only here so the sidebar does not grow a "
            "settings panel.")
        afk_info.setStyleSheet("color: #7a8290; font-size: 11px;")
        afk_info.setWordWrap(True)
        ac.addWidget(afk_info)

        # ---- preset switch (same idea as the text templates above) ----
        preset_row = QHBoxLayout()
        preset_row.setSpacing(4)
        preset_row.addWidget(QLabel("Text:"))
        self.afk_preset_group = QButtonGroup(self)
        self.afk_preset_group.setExclusive(True)
        self.afk_preset_buttons = []
        for i in range(AFK_PRESET_COUNT):
            b = QPushButton(str(i + 1))
            b.setCheckable(True)
            b.setFixedSize(30, 26)
            b.setCursor(Qt.CursorShape.PointingHandCursor)
            b.setToolTip(f"AFK text {i + 1} \u2013 its own line, kept "
            ...
            b.setStyleSheet(
                ...
                "QPushButton:checked { background: #5b8dc9;"
                " border-color: #5b8dc9; color: #ffffff; }")
            self.afk_preset_group.addButton(b, i)
            preset_row.addWidget(b)
            self.afk_preset_buttons.append(b)
        preset_row.addStretch()
        ac.addLayout(preset_row)
        self.afk_preset_group.idClicked.connect(self.on_afk_preset)

        preset_hint = QLabel(
            "Three texts, switched like the templates above \u2013 each keeps "
            "its own, so trying one out never overwrites another. Use "
            "\\n for a line break.<br><br>"
            "No frame is drawn around the text. VRChat's chatbox font is "
            "proportional, so box characters (\u2554\u2550\u2557 \u2551) "
            "do not line up there \u2013 they come out as loose segments "
            "floating around the words. If you want a frame, switch on the "
            "<b>Custom Box</b> card: it puts its top line above and its "
            "bottom line below, and it is built for exactly this.")
        preset_hint.setStyleSheet("color: #7a8290; font-size: 11px;")
        preset_hint.setWordWrap(True)
        ac.addWidget(preset_hint)

        # ---- the text itself ------------------------------------------
        afk_row_w = QWidget()
        afk_row = QHBoxLayout(afk_row_w)
        afk_row.setContentsMargins(0, 0, 0, 0)
        afk_row.setSpacing(6)
        afk_lbl = QLabel("Text:")
        afk_lbl.setFixedWidth(52)
        afk_row.addWidget(afk_lbl)
        self.afk_text_edit = QLineEdit()
        self.afk_text_edit.setMaxLength(CHATBOX_LIMIT - len(SLIM_SUFFIX))
        self.afk_text_edit.setToolTip(
            "Placeholders work here the same way they do in a status "
            "text, so {realtime} or a plugin value can sit in the line. "
            "\\n starts a new line. An empty field falls back to this "
            "slot's built-in text \u2013 being AFK with a blank chatbox is "
            "never what the switch was for.")
        self.afk_text_edit.textChanged.connect(self.on_afk_text)
        afk_row.addWidget(self.afk_text_edit, 1)
        afk_icon = QPushButton("\U0001F600")
        afk_icon.setObjectName("iconbtn")
        afk_icon.setFixedSize(30, 30)
        afk_icon.setToolTip("Insert icon")
        afk_icon.setCursor(Qt.CursorShape.PointingHandCursor)
        afk_icon.clicked.connect(
            lambda _, e=self.afk_text_edit, b=afk_icon:
            self.emoji_popup.open_for(e, b))
        afk_row.addWidget(afk_icon)
        self.afk_style_combo = self._make_style_combo(
            "How the AFK text is rendered in the chatbox")
        self.afk_style_combo.currentIndexChanged.connect(
            lambda _idx: self.on_afk_style(
                self.afk_style_combo.currentData()))
        afk_row.addWidget(self.afk_style_combo)
        self.afk_text_row = afk_row_w
        ac.addWidget(afk_row_w)

        # ---- timer ----------------------------------------------------
        timer_row = QHBoxLayout()
        timer_row.setSpacing(6)
        self.chk_afk_timer = QCheckBox("Show how long")
        self.chk_afk_timer.setToolTip(
            "Adds a line under the text counting how long you have been "
            "away.\n\n"
            "Counts in whole minutes on purpose: the chatbox re-sends "
            "whenever the text changes, and a seconds counter would put "
            "a message on the wire every few seconds for as long as you "
            "are gone \u2013 which VRChat answers with a blackout.\n\n"
            "Put {afk_time} in the text yourself and the extra line is "
            "skipped: you have already said where you want the counter.")
        self.chk_afk_timer.toggled.connect(self.on_afk_timer)
        timer_row.addWidget(self.chk_afk_timer)
        self.afk_timer_edit = QLineEdit()
        self.afk_timer_edit.setPlaceholderText(DEFAULT_AFK_TIMER_TEXT)
        self.afk_timer_edit.setMaxLength(CHATBOX_LIMIT - len(SLIM_SUFFIX))
        self.afk_timer_edit.setToolTip(
            "The line the timer adds. {afk_time} is replaced with "
            "\u201c12 min\u201d or \u201c1 h 05 min\u201d \u2013 and the "
            "wording around it is yours, because \u201cfor\u201d is "
            "English and this app is used in German every day.")
        self.afk_timer_edit.textChanged.connect(self.on_afk_timer_text)
        timer_row.addWidget(self.afk_timer_edit, 1)
        ac.addLayout(timer_row)

        self.chk_afk_solo = QCheckBox("Show the AFK text on its own")
        self.chk_afk_solo.setToolTip(
            "On (default): while you are away the chatbox says exactly "
            "one thing. Personal Status, the song, the hardware line and "
            "every plugin line are held back until you are back \u2013 which "
            "is what \"the text changes to AFK\" means.\n\n"
            "Off: the AFK text goes on top of your normal chatbox "
            "instead of replacing it.")
        self.chk_afk_solo.toggled.connect(self.on_afk_solo)
        ac.addWidget(self.chk_afk_solo)

        afk_param_row = QHBoxLayout()
        afk_param_row.setSpacing(6)
        afk_param_row.addWidget(QLabel("Parameter:"))
        self.afk_param_edit = QLineEdit()
        self.afk_param_edit.setPlaceholderText(DEFAULT_AFK_PARAM)
        self.afk_param_edit.setFixedWidth(160)
        self.afk_param_edit.setToolTip(
            "The avatar parameter Detect AFK listens to. VRChat sets "
            f"\"{DEFAULT_AFK_PARAM}\" itself, so this only needs changing "
            "for an avatar that carries its own away toggle under a "
            "different name. Empty falls back to the built-in one.")
        self.afk_param_edit.textChanged.connect(self.on_afk_param)
        afk_param_row.addWidget(self.afk_param_edit)
        afk_param_row.addStretch()
        ac.addLayout(afk_param_row)

        # ---- what it will look like -----------------------------------
        # The Custom Box lines are deliberately NOT shown here: this is
        # the AFK text, and whether anything is drawn around it is that
        # card's business, live in the big Preview.
        self.afk_preview = QLabel("")
        self.afk_preview.setStyleSheet(
            "font-size: 12px; color: #9fd49f; background: #0d0f13;"
            " border: 1px solid #2c313c; border-radius: 6px; padding: 8px;")
        self.afk_preview.setTextInteractionFlags(
            Qt.TextInteractionFlag.TextSelectableByMouse)
        ac.addWidget(self.afk_preview)

        self.afk_status = QLabel("")
        self.afk_status.setObjectName("dim")
        self.afk_status.setWordWrap(True)
        ac.addWidget(self.afk_status)
        sc.addWidget(self.afk_container)
        ...
        src_head = QLabel("Look for lyrics in:")
        lyr.addWidget(src_head)
        src_hint = QLabel(
            "Tried in this order, and the first synced hit wins \u2013 a "
            "source further down is only ever contacted when everything "
            "above it came back empty. Switching one on re-checks the "
            "songs that had no lyrics before.")
        src_hint.setStyleSheet("color: #7a8290; font-size: 11px;")
        src_hint.setWordWrap(True)
        lyr.addWidget(src_hint)

        self.lyrics_src_row = QWidget()
        src_grid = QGridLayout(self.lyrics_src_row)
        src_grid.setContentsMargins(20, 0, 0, 0)
        src_grid.setHorizontalSpacing(18)
        src_grid.setVerticalSpacing(4)
        self.lyrics_src_checks = {}
        for i, (sid, label, _on, blurb) in enumerate(SOURCES):
            box = QCheckBox(label)
            box.setToolTip(blurb)
            box.toggled.connect(
                lambda on, s=sid: self.on_lyrics_source(s, on))
            src_grid.addWidget(box, i // 2, i % 2)
            self.lyrics_src_checks[sid] = box
        lyr.addWidget(self.lyrics_src_row)
    ...
    # ----------------------------------------------------------- AFK
    def on_afk_expand(self):
        """Folds the AFK settings in/out."""
        show = self.afk_container.isHidden()
        self.afk_container.setVisible(show)
        self.afk_expander.setText(
            "\u25BE  Hide AFK text" if show else "\u25B8  Customize AFK text")
        if show:
            self.update_afk_status()

    def on_afk_detect_toggled(self, on):
        """Detect AFK.

        The parameter it reads only arrives when the OSC input is
        running, and that is opt-in because it binds udp/9001. So rather
        than leaving a switch that is on and does nothing, turning this
        on asks once - and a No puts the switch back instead of
        pretending it worked.
        """
        if getattr(self, "_block_updating", False):
            return
        if on and not self.cfg.get("osc_input_enabled"):
            if not self._offer_osc_input_for_afk():
                self._block_updating = True
                self.toggle_afk_detect.setChecked(False)
                self._block_updating = False
                return
        self.cfg["afk_detect"] = bool(on)
        self.save_config()
        self.log(f"Detect AFK: {'ON' if on else 'OFF'}")
        self.update_timers()
        self.afk_changed()

    def _offer_osc_input_for_afk(self):
        """Asks whether the OSC input may be switched on. True when it
        is running (or at least enabled) afterwards."""
        answer = QMessageBox.question(
            self, "Detect AFK",
            "Detect AFK reads VRChat's own AFK avatar parameter, which "
            "only arrives while \u201cReceive avatar parameters (OSC "
            "input)\u201d is on \u2013 that setting is off right now.\n\n"
            "Switch it on? It binds a UDP port (9001 by default); if "
            "another OSC tool already holds that port, pick a different "
            "one on the Options page and point VRChat at it.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.Yes)
        if answer != QMessageBox.StandardButton.Yes:
            self.log("Detect AFK: left off \u2013 it needs the OSC input.")
            return False
        self.cfg["osc_input_enabled"] = True
        self.toggle_osc_in.blockSignals(True)
        self.toggle_osc_in.setChecked(True)
        self.toggle_osc_in.blockSignals(False)
        self.save_config()
        self.update_osc_input()
        return True

    def on_afk_manual_toggled(self, on):
        if getattr(self, "_block_updating", False):
            return
        self.cfg["afk_manual"] = bool(on)
        self.save_config()
        self.log(f"I'm AFK: {'ON' if on else 'OFF'}")
        self.afk_changed()

    def on_afk_preset(self, index):
        """Switches to one of the three AFK texts.

        Nothing is copied between them: each keeps its own line, exactly
        the way each Personal Status template keeps its own set. Trying
        text 2 out and going back has to leave text 1 as it was.
        """
        index = min(AFK_PRESET_COUNT - 1, max(0, int(index)))
        self.cfg["afk_preset"] = index
        self.save_config()
        self.log(f"AFK text {index + 1} active")
        # re-fill the field from the slot that just came up - blocked,
        # because setText() would otherwise write straight back into the
        # slot we are switching away from
        self._block_updating = True
        self.afk_text_edit.setText(afk_text(self.cfg))
        self._block_updating = False
        self._sync_afk_dependents()
        self.afk_changed()

    def on_afk_text(self, text):
        if getattr(self, "_block_updating", False):
            return
        texts = self.cfg.setdefault("afk_texts", list(DEFAULT_AFK_TEXTS))
        while len(texts) < AFK_PRESET_COUNT:
            texts.append("")
        texts[afk_preset(self.cfg)] = text
        self.save_config_later()      # debounced: this is a text field
        self._afk_live_edit()

    def on_afk_timer(self, on):
        if getattr(self, "_block_updating", False):
            return
        self.cfg["afk_timer"] = bool(on)
        self.save_config()
        self._sync_afk_dependents()
        self.afk_changed()

    def on_afk_timer_text(self, text):
        if getattr(self, "_block_updating", False):
            return
        self.cfg["afk_timer_text"] = text
        self.save_config_later()
        self._afk_live_edit()

    def _afk_live_edit(self):
        """What every AFK text field does after a keystroke.

        The instant send is only asked for while you are actually away -
        editing the AFK text with the switches off changes nothing that
        is on screen in VRChat, and typing a sentence would otherwise be
        one send per character coalesced into a burst.
        """
        self.update_afk_status()
        self.update_preview()
        if self.afk_active():
    ...
    def on_afk_style(self, value):
        if getattr(self, "_block_updating", False):
            return
        self.cfg["afk_style"] = normalize_style(value)
        self.save_config()
        self.afk_changed()

    def on_afk_solo(self, on):
        if getattr(self, "_block_updating", False):
            return
        self.cfg["afk_solo"] = bool(on)
        self.save_config()
        self.afk_changed()

    def on_afk_param(self, text):
        if getattr(self, "_block_updating", False):
            return
        self.cfg["afk_param"] = text.strip()
        self.save_config_later()
        # the name changed, so whatever we thought we knew about the old
        # one is worthless - re-read before the next poll can compare
        self._afk_state = self.afk_active()
        self.update_afk_status()
        self.update_preview()

    def _sync_afk_dependents(self):
        """Keeps the preset buttons and the timer field in step."""
        index = afk_preset(self.cfg)
        button = self.afk_preset_buttons[index]
        if not button.isChecked():
            button.setChecked(True)
        self.afk_timer_edit.setEnabled(bool(self.cfg.get("afk_timer")))
        self.afk_text_edit.setPlaceholderText(DEFAULT_AFK_TEXTS[index])

    def afk_preview_text(self):
        """The AFK text as it will go out, whether or not you are away.

        Built from the same code path as the real one (see
        MainWindow.afk_line) with the timer showing a stand-in value, so
        what is on screen here cannot drift from what VRChat gets.
        """
        style = self.cfg.get("afk_style", STYLE_NORMAL)
        time_text = format_afk_time(
            self.afk_seconds() if self.afk_active() else 12 * 60)
        return "\n".join(self._render_status(line, style)
                          for line in afk_body(self.cfg, time_text))

    def update_afk_status(self):
        """The two lines under the AFK settings: what the box looks
        like, and - the part that actually costs people an evening -
        whether the detection has anything to read at all."""
        label = getattr(self, "afk_status", None)
        if label is None:
            return
        preview = self.afk_preview_text()
        if self.afk_preview.text() != preview:
            self.afk_preview.setText(preview)
        if self.cfg.get("afk_manual"):
            state = "\U0001F4A4 Away right now (I'm AFK is on)."
        elif not self.cfg.get("afk_detect"):
            state = "Both switches are off \u2013 nothing is sent."
        elif not getattr(self.osc_in, "running", False):
            state = ("\u26A0 Detect AFK is on, but the OSC input is not "
                     "running \u2013 switch on \u201cReceive avatar "
                     "parameters\u201d on the Options page.")
        elif self.afk_param_value() is None:
            state = (f"\u23F3 Listening on udp/{self.osc_in.port} \u2013 no "
                     f"\u201c{afk_param_name(self.cfg)}\u201d parameter has "
                     "arrived yet. VRChat sends it once you are in a "
                     "world with OSC enabled.")
        elif self.afk_detected():
            state = "\U0001F4A4 VRChat says you are away."
        else:
            state = "\u2705 VRChat says you are here."
        used = len(preview)
        if used > CHATBOX_LIMIT:
            state += (f"\n\u26A0 {used}/{CHATBOX_LIMIT} characters \u2013 "
                      "this text alone is too long for the chatbox and "
                      "will be cut off.")
        else:
            state += f"\n{used}/{CHATBOX_LIMIT} characters."
        if label.text() != state:
    ...
    def advance_status(self):
        ...
        if self.afk_holds_the_chatbox():
            # away, and the AFK line is the entire chatbox - the text
            # rotating behind it is invisible, so take the switch and
            # send nothing (see afk_holds_the_chatbox)
            self.commit_status()
            return
    ...
    def _sync_media_dependents(self):
        ...
        self.lyrics_src_row.setVisible(lyrics_on)
    ...
    def on_lyrics_source(self, source_id, on):
        ...
        The order is not stored - normalize_sources() takes it from the
        list in core/lyrics_sources.py every time, so a hand-edited
        config cannot end up asking the slow unofficial endpoint first.
        """
        if getattr(self, "_block_updating", False):
            return
        wanted = set(self.cfg.get("media_lyrics_sources") or [])
        wanted.add(source_id) if on else wanted.discard(source_id)
        self.cfg["media_lyrics_sources"] = normalize_sources(wanted)
        self.save_config()
        self.log(f"Lyrics sources: "
                 f"{', '.join(self.cfg['media_lyrics_sources']) or 'none'}")
        self._sync_lyrics_sources()

    def _sync_lyrics_sources(self):
        """Pushes the tick boxes into the fetcher."""
        self.lyrics.set_sources(self.cfg.get("media_lyrics_sources"))
    ...
    def advance_aio(self):
        ...
        if not self.aio_is_advanced():
            ...
            # send the same text a second time. Skipped while the AFK
            # line owns the chatbox, for the same reason.
            if not self.afk_holds_the_chatbox():
                self.request_send()
