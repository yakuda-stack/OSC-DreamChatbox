...
class MainWindow(ConfigMixin, AppsPageMixin, AdvancedPageMixin,
    ...
    def __init__(self):
        ...
        self.aio_index = 0
        # AFK (core/afk.py). The last state poll_afk() acted on, so a
        # parameter that keeps arriving with the same value costs one
        # dict lookup a second and nothing else - entering and leaving
        # AFK is the only thing worth a send.
        self._afk_state = False
        # when the current away stretch began, for {afk_time}. None
        # while present; set lazily by afk_seconds() so a payload built
        # before the poller has run still gets a sane number instead of
        # a zero that jumps.
        ...
        self.afk_timer.timeout.connect(self.poll_afk)
    ...
    def build_ui(self):
        ...
        # ---- AFK, inside the Preview box -----------------------------
        # These two decide WHAT the preview above them is showing, so
        # they belong to it rather than to the column of app switches.
        # Small toggles and short labels on purpose: this is a footer,
        # not a third section - the settings behind it live at the
        # bottom of the Personal Status card.
        afk_sep = QFrame()
        afk_sep.setFrameShape(QFrame.Shape.HLine)
        afk_sep.setObjectName("hline")
        pv_layout.addWidget(afk_sep)

        afk_row = QHBoxLayout()
        afk_row.setContentsMargins(0, 0, 0, 0)
        afk_row.setSpacing(6)
        self.toggle_afk_detect = ToggleSwitch(small=True)
        afk_detect_tip = (
            "Detect AFK\n\n"
            "Notices when VRChat says you are away - headset off, or the "
            "window left alone - and switches the chatbox over to your "
            "AFK box by itself. Switches back the moment you are active "
            "again.\n\n"
            "Reads VRChat's own AFK avatar parameter, so it needs "
            "\u201cReceive avatar parameters (OSC input)\u201d on the "
            "Options page. Turning this on offers to switch that on for "
            "you.")
        self.toggle_afk_detect.setToolTip(afk_detect_tip)
        self.toggle_afk_detect.toggled.connect(self.on_afk_detect_toggled)
        lbl_afk_detect = ToggleLabel("Detect", self.toggle_afk_detect)
        lbl_afk_detect.setObjectName("dim")
        lbl_afk_detect.setToolTip(afk_detect_tip)
        afk_row.addWidget(self.toggle_afk_detect)
        afk_row.addWidget(lbl_afk_detect)
        afk_row.addStretch()

        self.toggle_afk_manual = ToggleSwitch(small=True)
        afk_manual_tip = (
            "I'm AFK\n\n"
            "Says you are away for exactly as long as this is on - no "
            "detection involved. Use it when you are at the keyboard but "
            "not really there.\n\n"
            "Wins over Detect AFK: if you flipped this on, you meant it.")
        self.toggle_afk_manual.setToolTip(afk_manual_tip)
        self.toggle_afk_manual.toggled.connect(self.on_afk_manual_toggled)
        lbl_afk_manual = ToggleLabel("I'm AFK", self.toggle_afk_manual)
        lbl_afk_manual.setObjectName("dim")
        lbl_afk_manual.setToolTip(afk_manual_tip)
        afk_row.addWidget(self.toggle_afk_manual)
        afk_row.addWidget(lbl_afk_manual)
    ...
    def switch_page(self, idx):
        ...
        for i, b in enumerate(self.nav_buttons):
            b.setChecked(i == idx)
        if idx == self.PAGE_APPS:
            # the AFK line reports whether VRChat is talking to us at
            # all, and that can change while another page is open
            self.update_afk_status()
        elif idx == self.PAGE_ADVANCED:
    ...
    def apply_config_to_ui(self):
        ...
        # blockSignals, not the _block_updating flag: on_afk_detect_toggled
        # can open a dialog asking to switch the OSC input on, and a
        # question box during startup is not a thing anyone asked for.
        self.toggle_afk_detect.blockSignals(True)
        self.toggle_afk_detect.setChecked(bool(self.cfg.get("afk_detect")))
        self.toggle_afk_detect.blockSignals(False)
        self.toggle_afk_manual.blockSignals(True)
        self.toggle_afk_manual.setChecked(bool(self.cfg.get("afk_manual")))
        self.toggle_afk_manual.blockSignals(False)
        self.chk_afk_solo.setChecked(bool(self.cfg.get("afk_solo", True)))
        self.chk_afk_timer.setChecked(bool(self.cfg.get("afk_timer")))
        self.afk_timer_edit.setText(str(self.cfg.get("afk_timer_text", "")))
        self.afk_text_edit.setText(afk_text(self.cfg))
        self.afk_param_edit.setText(str(self.cfg.get("afk_param", "")))
        self.set_style_combo(self.afk_style_combo,
                             self.cfg.get("afk_style", STYLE_NORMAL))
        active = set(self.cfg.get("media_lyrics_sources") or [])
        for sid, box in self.lyrics_src_checks.items():
            box.blockSignals(True)
            box.setChecked(sid in active)
    ...
    # ------------------------------------------------------------- AFK
    def afk_param_value(self):
        """The raw AFK parameter, or None when none has arrived.

        None is deliberately distinct from False: "VRChat has not told
        us anything" and "VRChat says you are here" look identical in
        the chatbox but need completely different help text, and this is
        the only place that can still tell them apart.
        """
        listener = getattr(self, "osc_in", None)
        if listener is None or not listener.running:
            return None
        return listener.get(afk_param_name(self.cfg))

    def afk_detected(self):
        """True when Detect AFK is on AND the parameter says away."""
        if not self.cfg.get("afk_detect"):
            return False
        return is_afk_value(self.afk_param_value())

    def afk_active(self):
        """The one answer the rest of the app asks for.

        The manual switch is checked first and short-circuits, so
        "I'm AFK" works with the OSC input off, with VRChat closed, and
        while Detect AFK disagrees.
        """
        return bool(self.cfg.get("afk_manual")) or self.afk_detected()

    def afk_seconds(self):
        """How long the current away stretch has been running.

        The start time is set here when it is missing rather than only
        by the poller, so a payload built the instant the switch is
        flipped already has a starting point. Nothing depends on the
        exact moment - the timer counts in minutes - but a number that
        silently restarts at zero every frame would.
        """
        if not self.afk_active():
            self._afk_since = None
            return 0.0
        if self._afk_since is None:
            self._afk_since = time.time()
        return max(0.0, time.time() - self._afk_since)

    def afk_line(self):
        """The AFK text, or "" when not away.

        Each line goes through _render_status() on its own, so a
        placeholder ({realtime}, a plugin value) works inside it and the
        small-letter styles apply the same way they do to a status text.

        Nothing is drawn around it. VRChat's chatbox font is
        proportional, so box characters render as loose segments rather
        than as a frame - framing is the Custom Box card's job, and
        build_payload runs the payload through it like any other.
        """
        if not self.afk_active():
            return ""
        style = self.cfg.get("afk_style", STYLE_NORMAL)
        time_text = format_afk_time(self.afk_seconds())
        lines = [self._render_status(line, style)
                 for line in afk_body(self.cfg, time_text)]
        return "\n".join(line for line in lines if line.strip())

    def afk_holds_the_chatbox(self):
        """True while the AFK line IS the whole payload.

        The two rotation timers ask before their instant send. A status
        text or an AIO string switching behind the AFK line changes
        nothing VRChat can see, so sending it would put the identical
        message on the wire again - and every one of those spends part
        of the chatbox rate limit that the "I'm back" message needs.
        The rotation itself keeps running, so coming back continues
        where it left off instead of restarting.
        """
        return self.afk_active() and bool(self.cfg.get("afk_solo", True))

    def poll_afk(self):
        """One second of Detect AFK.

        Only reacts to a *change*: sending the same AFK text again every
        second would spend VRChat's whole chatbox budget on a message
        that is already on screen.
        """
        state = self.afk_active()
        if state == self._afk_state:
            return
    ...
    def afk_changed(self):
        ...
        request_send() rather than send_now(): going AFK is exactly the
        kind of change instant send exists for, and it still has to
        queue behind the rate limit like everything else.
        """
        state = self.afk_active()
        if not state:
            self._afk_since = None
    ...
    def update_timers(self):
        ...
        # AFK detection. One dict lookup a second, and only while the
        # switch that needs it is on - the manual one changes state by
        # being clicked and tells us itself.
        if self.cfg.get("afk_detect"):
    ...
    def build_payload(self, commit=False):
        ...
        if chat_lines:
            anchored[chat_anchor] = (list(anchored.get(chat_anchor, []))
                                     + chat_lines)
        # AFK, decided AFTER the automation above ran: a canvas that
        # presses a hotkey or starts a program has nothing to do with
        # whether you are at the desk, so being away must not stop it.
        # Only the text is taken over.
        afk = self.afk_line()
        if afk and self.cfg.get("afk_solo", True):
            # the whole point of the switch: while you are gone the
            # chatbox says one thing, not one thing plus a song plus
            # your GPU temperature
            # through the Custom Box like any other payload: it adds a
            # top and a bottom line only when that card is switched on,
            # which is exactly where a frame around the AFK text should
            # come from
            return self.plugins.filter_text(
                "\n".join(self._apply_custom_box(afk.split("\n"))))
