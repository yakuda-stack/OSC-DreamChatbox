...
class ConfigMixin:
    def load_config(self):
        ...
        defaults = {
            ...
            "status_active": True,
            # ---- AFK (core/afk.py) ------------------------------------
            # Two switches above SendToVRChat. Both off by default:
            # nothing about an existing setup changes on update, and
            # neither one is something to discover by finding your
            # chatbox saying you are away.
            #
            # Detection reads VRChat's own AFK avatar parameter, which
            # arrives over the OSC input - so turning it on offers to
            # switch that on too rather than sitting there doing
            # nothing (see AppsPageMixin.on_afk_detect_toggled).
            "afk_detect": False,
            "afk_manual": False,
            "afk_param": DEFAULT_AFK_PARAM,
            # Three switchable texts, the way the Personal Status
            # templates are - and like those, each one keeps its own
            # text, so trying preset 2 out never overwrites preset 1.
            # The fields live at the bottom of the Personal Status card;
            # the two switches themselves sit under the Preview, where
            # you can reach them mid-session.
            #
            # No frame is drawn around them: VRChat's chatbox font is
            # proportional and box characters do not line up in it. The
            # Custom Box card does the framing, and only when it is on.
            "afk_preset": 0,
            "afk_texts": list(DEFAULT_AFK_TEXTS),
            # counts how long you have been gone and puts it under the
            # text. Off by default: a line that changes is a line that
            # costs a chatbox send every time it does.
            "afk_timer": False,
            "afk_timer_text": DEFAULT_AFK_TIMER_TEXT,
            "afk_style": STYLE_NORMAL,
            # True: the AFK line IS the chatbox, everything else is held
            # back while you are gone - which is what "the text changes
            # to AFK" means. False keeps the normal payload and puts the
            # AFK line on top of it.
            "afk_solo": True,
            "media_active": False,
            ...
            # Which lyrics services may be asked, in lookup order (see
            # core/lyrics_sources.py). LRCLIB first and always; the rest
            # are only ever contacted for songs it does not have, so a
            # song it knows costs exactly what it did before.
            "media_lyrics_sources": list(DEFAULT_SOURCES),
        ...
        # untouched. The text is capped the same way a status text is:
        # it is a chatbox line and the field enforces the same limit, so
        # a hand-edited config cannot smuggle a longer one past it.
        for key in ("afk_detect", "afk_manual", "afk_timer", "afk_solo"):
            defaults[key] = bool(defaults.get(key))
        param = defaults.get("afk_param", DEFAULT_AFK_PARAM)
        defaults["afk_param"] = (str(param).strip()
                                 if isinstance(param, str) else "") \
            or DEFAULT_AFK_PARAM
        limit = CHATBOX_LIMIT - len(SLIM_SUFFIX)
        try:
            preset = int(defaults.get("afk_preset", 0))
        except (TypeError, ValueError):
            preset = 0
        defaults["afk_preset"] = preset if 0 <= preset < AFK_PRESET_COUNT else 0
        texts = defaults.get("afk_texts")
        if not isinstance(texts, list):
            texts = []
        texts = [str(t or "")[:limit] for t in texts][:AFK_PRESET_COUNT]
        texts += list(DEFAULT_AFK_TEXTS[len(texts):])
        # The first two releases of the AFK block stored the text
        # differently: one field plus a "use my own" checkbox, then three
        # framed presets in a dict. Both only survive one load - they are
        # popped here - and whatever they held lands on the third preset,
        # which is the free-form one either way. The frames themselves are
        # gone: they never lined up in VRChat's proportional chatbox font.
        legacy_text = defaults.pop("afk_text", None)
        defaults.pop("afk_custom", None)
        legacy_boxes = defaults.pop("afk_boxes", None)
        defaults.pop("afk_frame", None)
        carried = ""
        if isinstance(legacy_boxes, dict):
            box = legacy_boxes.get("custom")
            if isinstance(box, dict):
                carried = str(box.get("text", "") or "").strip()
        if not carried and isinstance(legacy_text, str):
            carried = legacy_text.strip()
        if carried:
            texts[AFK_PRESET_COUNT - 1] = carried[:limit]
            defaults["afk_preset"] = AFK_PRESET_COUNT - 1
        defaults["afk_texts"] = texts
        defaults["media_lyrics_sources"] = normalize_sources(
            defaults.get("media_lyrics_sources"))
        timer_text = defaults.get("afk_timer_text", DEFAULT_AFK_TIMER_TEXT)
        if not isinstance(timer_text, str) or not timer_text.strip():
            timer_text = DEFAULT_AFK_TIMER_TEXT
        defaults["afk_timer_text"] = timer_text[:limit]
