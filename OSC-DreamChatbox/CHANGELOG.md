...
that says so because you said so.**

### Added

**AFK**

- **Detect** watches VRChat's own `AFK` avatar parameter — the one
  VRChat sets when you take the headset off or leave the window alone —
  and swaps the chatbox over by itself. It swaps back the moment you are
  active again. Nothing is guessed from idle timers: the game has already
  made that decision and it makes it correctly in desktop mode too.
- **I'm AFK** is the manual version. No detection, no parameter, no
  VRChat required: the chatbox says you are away for exactly as long as
  the switch is on. It **wins over Detect** — somebody who flips it on
  while VRChat still thinks they are present meant it.
- Both live at the **bottom of the Preview box**, small and dim. They
  decide what the preview above them is showing, so they belong to it
  rather than to the column of app switches — and putting them there
  keeps that column at the length it already was.

**Three switchable texts**

- Switched the way the Personal Status templates are, and for the same
  reason: trying text 2 out and going back has to leave text 1 saying
  what it said. Nothing is copied between them. `\n` is a line break,
  the same two-character spelling All in one uses.
- Placeholders work inside the text, and the small-letter styles apply
  to it the way they do to a status text.
- **No frame is drawn.** VRChat's chatbox font is proportional, so
  box-drawing characters do not line up in it — a `╔═══╗` / `║ … ║`
  frame renders as loose, unconnected segments floating around the
  words rather than as a box. Framing is the **Custom Box** card's job:
  it already puts a top line above and a bottom line below the finished
  payload, and only when the user has switched it on. The AFK payload
  goes through it like any other.

**A timer, off by default**

- **Show how long** adds `{afk_time}` on its own line under the text —
  `for 12 min`, `for 1 h 05 min`. The wording around it is editable,
  because *"for"* is English and this app is used in German every day.
- It counts in **whole minutes on purpose.** The chatbox re-sends
  whenever the text changes, so a seconds counter would put a message on
  the wire every few seconds for as long as you are gone — and VRChat
  answers a burst of those with a ~30 second blackout. A number that
  moves once a minute says the same thing and costs one extra send an
  hour.
- Put `{afk_time}` in the text yourself and the extra line is skipped:
  you have already said where you want the counter, and a second copy
  underneath would be the app arguing.
- The line is **appended, not woven in**: switching the timer off leaves
  your text exactly as it was.

**The detection needs the OSC input, and says so instead of sitting
there**

- The `AFK` parameter only arrives while *Receive avatar parameters (OSC
  input)* is on, and that is opt-in because it binds udp/9001. Turning
  Detect on therefore **asks once** whether to switch it on. A *No* puts
  the switch back off rather than leaving one that is on and does
  nothing.

**Everything about the text lives at the bottom of Personal Status**

- Folded away under *Customize AFK text*, so the Preview footer stays two
  small switches and does not grow a settings panel.
- Includes a live preview, a character count against VRChat's 144, and a
  status line reporting what detection can actually see: waiting for the
  OSC input · listening but nothing has arrived yet · VRChat says you are
  here · VRChat says you are away. That last distinction is the part that
  otherwise costs somebody an evening.

**It does not spend the chatbox rate limit while you are gone**

- With the AFK text owning the chatbox, a Personal Status text or an AIO
  string rotating behind it is invisible — so the instant send for those
  rotations is skipped. The rotation itself keeps running, so coming back
  continues where it left off instead of restarting at Text 1.

**Off by default, in every direction**

- Every one of the new keys is absent from existing configs and falls
  back to off, so nothing about a current setup changes on update — and
  nobody discovers the feature by finding their chatbox saying they are
  away.
- Nineteen tests cover the rules: parameter values a bridge might send
  (bool, float, int, `"on"`, `"away"`), *nothing heard yet* staying
  distinct from *the player is here*, manual beating detection, a renamed
  parameter, each slot keeping its own text, the empty-slot fallback, the
  timer's minute granularity, a hand-placed counter not being doubled,
  and the rotation standing down while AFK owns the chatbox.

**MediaPlay: five more places to look for lyrics**

- LRCLIB alone misses a lot — anything released in the last few weeks,
  anything with a non-Latin title, and most of what plays through
  YouTube with a third-party upload's metadata. Five more sources are
  now available, all key-less:

  | Source | Default | Why |
  |---|---|---|
  | **LRCLIB** | on | the open database, unchanged |
  | **LyricsPlus** | on | aggregator, asks Apple/Musixmatch/Spotify at once |
  | **Better Lyrics** | on | Apple's TTML, tightest timings in the list |
  | **Paxsenix** | off | Apple Music catalogue, two round trips |
  | **KuGou** | off | enormous CJK catalogue, thin elsewhere |
  | **Musixmatch** | off | what Spotify shows, via a guest token |

- **Tried in that order, first synced hit wins.** A source further down
  is only ever contacted when everything above it came back empty, so a
  song LRCLIB knows costs exactly the one request it always did. Your
  own `.lrc` files still beat all six.
- The order is taken from the source list in code, never from the
  config — a hand-edited file cannot put the slow unofficial endpoint in
  front of the fast open one.
- Ticking a source **clears the negative cache**, so the songs that came
  back empty are re-checked instead of staying empty until restart.
- One source failing never takes the chain down with it; the failure is
  logged by name and the next one is tried.
- Paxsenix's search hits go through the **same scoring** the LRCLIB ones
  do — title-prefix compatibility plus a ±10 s duration window — before
  a second request is spent on them. Taking the first result blindly is
  how you end up with a karaoke cover's timings under the original.
- Musixmatch's guest token is fetched once behind a lock and reused for
  the process: several songs resolving at once, each asking for its own
  token, is the fastest way to get rate-limited.
- Everything is stdlib — `urllib`, `json`, `base64`, `ElementTree`. No
  new dependency, and the AppImage does not grow.
- The endpoint list is derived from
  [Meld](https://github.com/FrancescoGrazioso/Meld) (GPL-3.0).
- Seventeen tests cover the format plumbing: millisecond → `[mm:ss.cc]`
  conversion, minutes not wrapping past an hour, TTML in both its clock
  and offset time spellings, word timings flattened into the line,
  malformed TTML returning empty instead of raising, and the source
  order surviving a scrambled config.

### Changed

- `ToggleSwitch` grew a `small=True` size for switches that sit inside
  another block instead of heading one. A size, not a second widget, so
