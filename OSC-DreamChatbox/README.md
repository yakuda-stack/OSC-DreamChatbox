...
- At the bottom: **Customize AFK text** — the three texts and the timer of the [AFK](#-afk) feature live here, folded away so the sidebar stays short
...
Two small switches at the **bottom of the Preview box** — they decide what the preview above them is showing, so they sit with it rather than in the column of app switches.

| Switch | What it does |
|---|---|
| **Detect** | Watches VRChat's own `AFK` avatar parameter — set when you take the headset off or leave the window alone — and swaps the chatbox over by itself. Swaps back the moment you are active again. |
| **I'm AFK** | No detection. The chatbox says you are away for exactly as long as the switch is on. Wins over Detect. |

Three switchable texts, the way the Personal Status templates are — each keeps its own, so trying one out never overwrites another. With the timer on:

```
💤 AFK / Away 💤          💤 AFK
for 12 min                BRB – briefly away
                          for 12 min
```

**No frame is drawn around it.** VRChat's chatbox font is proportional, so box-drawing characters (`╔═╗ ║`) do not line up there — they come out as loose segments floating around the words. If you want a frame, switch on the **Custom Box** card: it puts its top line above and its bottom line below, and it is built for exactly that.

<details>
<summary><b>Details</b></summary>

- **Detect needs the OSC input.** The `AFK` parameter only arrives while *Receive avatar parameters (OSC input)* is on (Options page) — that binds udp/9001, which is why it is opt-in. Turning Detect on **asks once** whether to switch it on; a *No* puts the switch back rather than leaving one that does nothing.
- Nothing is guessed from idle timers. VRChat has already decided, and it decides correctly in desktop mode too.
- Everything about the text lives under **Customize AFK text** at the bottom of the Personal Status card:
  - **Text 1 · 2 · 3** with icon picker and size style. **Placeholders work**, so `{realtime}` or a plugin value can sit in the line, and `\n` starts a new one. An empty slot falls back to its built-in text.
  - **Show how long** adds `{afk_time}` on its own line — `for 12 min`, `for 1 h 05 min`. It counts in **whole minutes** on purpose: the chatbox re-sends whenever the text changes, and a seconds counter would put a message on the wire every few seconds for as long as you are gone, which VRChat answers with a blackout. The wording around it is editable, because *"for"* is English. Put `{afk_time}` in the text yourself and the extra line is skipped — you have already said where you want it.
  - **Show the AFK text on its own** (default **on**) — while you are away the chatbox says exactly one thing; Personal Status, the song, the hardware line and every plugin line are held back until you are back. Off puts the AFK text on top of your normal chatbox instead.
  - **Parameter** — for an avatar carrying its own away toggle under a different name. Empty falls back to `AFK`.
  - A live preview, a character count against VRChat's 144, and a status line reporting what detection can actually see: waiting for the OSC input · listening but nothing has arrived yet · VRChat says you are here · VRChat says you are away.
- **It does not spend the chatbox rate limit while you are gone.** With the AFK text owning the chatbox, a status text or AIO string rotating behind it is invisible, so those instant sends are skipped. The rotation keeps running, so coming back continues where it left off.
- Both switches are **off by default** — nothing about an existing setup changes on update.
...
| `lyricsplus.binimum.org` *(+2 mirrors)* | lyrics, when LRCLIB has nothing | only with lyrics on and LyricsPlus ticked (default) |
| `lyrics-api.boidu.dev` | lyrics, when the two above have nothing | only with lyrics on and Better Lyrics ticked (default) |
| `lyrics.paxsenix.org` | lyrics from the Apple Music catalogue | only when you tick Paxsenix |
| `lyrics.kugou.com`, `mobileservice.kugou.com` | lyrics, large CJK catalogue | only when you tick KuGou |
