...
# whenever you take the headset off or leave the window alone, so it is
# a built-in and arrives no matter what the avatar looks like. It stays
# configurable (see core/afk.py) for avatars that carry their own away
# toggle under a different name.
DEFAULT_AFK_PARAM = "AFK"

# Three switchable texts, the way Personal Status has switchable
# templates - each keeps its own text, so trying one out never
# overwrites another.
#
# No frame is drawn around them. VRChat's chatbox font is proportional,
# so box-drawing characters do NOT line up there: a ╔═══╗ / ║ … ║ frame
# comes out as loose, unconnected line segments floating around the
# text. Framing is the Custom Box card's job, and it only happens when
# that card is switched on - see CustomBoxMixin._apply_custom_box.
AFK_PRESET_COUNT = 3
DEFAULT_AFK_TEXTS = [
    "\U0001F4A4 AFK / Away \U0001F4A4",
    "\U0001F4A4 AFK \\n BRB \u2013 briefly away",
    "\U0001F4A4   AFK   \U0001F4A4",
]
#: the line the timer adds under the text. Editable, because "for" is
#: English and this app is used in German every day.
DEFAULT_AFK_TIMER_TEXT = "for {afk_time}"
